		...
		lock = {
			...
			set = function(info, v)
				...
				for k in pairs(bars) do
					...
					if
						v
						or not ClassTimer.db.profile.Units[unit].enable
						or (ClassTimer.db.profile.Group[unit] and unit ~= ClassTimer.db.profile.AllInOneOwner)
					then
				...
				AllInOne = {
					...
					args = {
						Units = {
							...
							get = function(_, key)
								return ClassTimer.db.profile.Group[key]
							end,
						...
						Owner = {
							...
							get = function()
								return ClassTimer.db.profile.AllInOneOwner
							end,
							set = function(_, value)
								ClassTimer.db.profile.AllInOneOwner = value
								ClassTimer.db.profile.Group[value] = true
							end,
							order = 3,
						},
					},
...
function ClassTimer:AddUnitOptions(type)
	...
	path.args[type] = {
		...
		childGroups = "tab",
		hidden = function()
			return not ClassTimer.db.profile.Units[type].enable
		end,
		get = function(info)
					...
					sizes = {
						...
						args = {
							...
							maxTime = {
								...
								--bigStep = 1,
								hidden = function()
									return not ClassTimer.db.profile.Units[type].sizeEnable
								end,
								arg = { type, "sizeMax" },
							},
						},
			...
			Texts = {
				...
				args = {
					...
					font = {
						...
						values = function()
							return ClassTimer:List(sm:List("font"))
						end,
						arg = { type, "font" },
						order = 4,
					},
			...
			Textures = {
				...
				args = {
					texture = {
						...
						values = function()
							return ClassTimer:List(sm:List("statusbar"))
						end,
						order = 1,
						arg = { type, "texture" },
					},
					...
					Debuffs = {
						...
						args = {
							...
							Curse = {
								...
								arg = { type, "Cursecolor" },
								hidden = function()
									return not ClassTimer.db.profile.Units[type].differentColors
								end,
							},
							Poison = {
								type = "color",
